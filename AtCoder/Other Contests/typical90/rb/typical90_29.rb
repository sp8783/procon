...
# 演算、マッピング、合成
op = Proc.new { |x, y| [x, y].max } # 最大値演算
mapping = Proc.new { |f, x| [f, x].max } # 遅延値を適用
composition = Proc.new { |f, g| [f, g].max } # 遅延値の結合

# Lazy Segment Tree の初期化
lst = LazySegtree.new(a, 0, 0, op, mapping, composition)

# 区間の更新
n.times do
  l, r = gets.split.map(&:to_i)  
  h = lst.prod(l - 1, r) + 1  # 区間の最大値 + 1
  lst.range_apply(l - 1, r, h) # 区間にhで更新
  puts h
end
